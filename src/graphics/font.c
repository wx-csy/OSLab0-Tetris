...
static const uint8_t ascii_bitmap[256][16] = {
  [0x20] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},  // space 0x20
...
void gDrawString(int x, int y, const char *str, gRGB_t color) {
  while (*str) {
    for (int j = 0; j < 12; j++) {
      uint8_t cur = ascii_bitmap[*str][j];
      for (int i = 0; i < 8; i++) {
        if (cur & (1 << i))
          _gPixel(x + i, j) = color;
      }
